    ...
    public class ClienteService : IClienteService
    {
        ...
        public async Task<BaseResponse> AddAsync(CreateClienteRequest request)
        {
            ...
            catch (Exception ex)
            {
                ...
                response.Mesagge = "Ocurrió un error al agregar cliente.";
                _logger.LogError(ex, "{0} - {1}", response.Mesagge, ex.Message);
            }
            return response;
        }
        ...
        public async Task<BaseResponse> UpdateAsync(int id, UpdateClienteRequest request)
        {
            ...
            try
            {
                ...
                if (cliente is null)
                {
                    ...
                    response.Mesagge = "Cliente no encontrado.";
                    return response;
                }
            ...
            {
                response.IsSucess = false;
                response.Mesagge = "Ocurrió un error al actualizar cliente.";
                _logger.LogError(ex, "{0} - {1}", response.Mesagge, ex.Message);
            }
            return response;
        }
        ...
        public async Task<BaseResponse<GetClienteResponse>> GetByIdAsync(int id)
        {
            ...
            {
                response.IsSucess = false;
                response.Mesagge = "Ocurrió un error al obtener cliente.";
                _logger.LogError(ex, "{0} - {1}", response.Mesagge, ex.Message);
            }
            return response;
        }
        ...
        public async Task<PagedResponse<ListClienteResponse>> ListAsync(SearchListRequest request)
        {
            ...
            {
                response.IsSucess = false;
                response.Mesagge = "Ocurrió un error al listar clientes.";
                _logger.LogError(ex, "{0} - {1}", response.Mesagge, ex.Message);
            }
            return response;
        }
        ...
        public async Task<BaseResponse> DeleteAsync(int id)
        {
            ...
            {
                response.IsSucess = false;
                response.Mesagge = "Ocurrió un error al eliminar cliente.";
                _logger.LogError(ex, "{0} - {1}", response.Mesagge, ex.Message);
            }
            return response;
        }
